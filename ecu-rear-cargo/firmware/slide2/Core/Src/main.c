...
#define PINCH_CURRENT_MA  500      /* TODO(calibration): 끼임 판정 전류 임계값(mA) */
  ...
  switch (rx.StdId) {
    ...
    case MODEL_CAR_NET_REAR_LEFT_SEAT_CMD_FRAME_ID: {  /* 0x120: 좌석 좌 */
      ...
      if (model_car_net_rear_left_seat_cmd_unpack(&m, d, sizeof(d)) == 0) {
        ...
        if (m.rl_slide_position == SLIDE_REZERO_CMD)
          slide_rezero();                              /* 255 = 현재 위치를 0점으로 재설정 */
        else if (m.rl_slide_position == SLIDE_HOME_CMD)
          slide_seek_home();                           /* 254 = 오른쪽 끝(시작점)으로 호밍 */
        else
          slide_set_target_mm(m.rl_slide_position);    /* ← 우리가 구동하는 슬라이드 축 */
      }
      break;
    }
    ...
    case MODEL_CAR_NET_REAR_RIGHT_SEAT_CMD_FRAME_ID: { /* 0x121: 좌석 우 */
      ...
      if (model_car_net_rear_right_seat_cmd_unpack(&m, d, sizeof(d)) == 0) {
        ...
        /* m.rr_slide_position → 우측 2번째 축(하드웨어 생기면) */
      }
      break;
    }
  ...
  if (ina_ok_rl) {
    ...
    if (i > PINCH_CURRENT_MA) {
      if (++cntRL >= PINCH_DEBOUNCE) {
        cntRL = 0;
        pinch_rl = 1;
        uint8_t deg = servo_get_deg(SERVO_RL);
        servo_set_deg(SERVO_RL, (deg > PINCH_BACKOFF_DEG) ? (deg - PINCH_BACKOFF_DEG) : 0);
      }
    } else {
      cntRL = 0;
    }
  ...
  if (ina_ok_rr) {
    ...
    if (i > PINCH_CURRENT_MA) {
      if (++cntRR >= PINCH_DEBOUNCE) {
        cntRR = 0;
        pinch_rr = 1;
        uint8_t deg = servo_get_deg(SERVO_RR);
        servo_set_deg(SERVO_RR, (deg > PINCH_BACKOFF_DEG) ? (deg - PINCH_BACKOFF_DEG) : 0);
      }
    } else {
      cntRR = 0;
    }
