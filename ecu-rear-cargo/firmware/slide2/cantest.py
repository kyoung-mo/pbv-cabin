...
def on_rx(m):
    count[m.arbitration_id] = count.get(m.arbitration_id, 0) + 1
    h = m.data.hex()
    if last.get(m.arbitration_id) != h:
        last[m.arbitration_id] = h
        print(f"\nRX  0x{m.arbitration_id:03X}  {h}")
...
rr = [0, 0, 0]   # 0x121 : [RR_Recline(0~180°), RR_Slide(예약), Cargo_Lamp]

def clamp(v, lo, hi):
    return lo if v < lo else hi if v > hi else v

HELP = ("명령:  h=호밍(시작점복귀)  m<mm>슬라이드(0~100)  z=재영점(현위치를0)  "
    ...
    while True:
        ...
        try:
            if c == "q":
                ...
                rl[1] = clamp(int(c[1:]), 0, 100); tx(0x120, rl)   # 슬라이드 위치
            elif c.startswith("a"):
