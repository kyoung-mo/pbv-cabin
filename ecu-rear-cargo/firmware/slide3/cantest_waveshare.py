...
# Waveshare USB-CAN-A (CH340) 용 cantest.
#   slcan(CANable)이 아니라 Seeed USB-CAN Analyzer와 동일 프로토콜이므로
#   python-can의 "seeedstudio" 인터페이스를 쓴다. (CANable 버전은 cantest.py 그대로)
PORT = "COM8"   # ← 장치관리자의 USB-SERIAL CH340 포트
bus = can.Bus(interface="seeedstudio", channel=PORT,
              baudrate=2000000,        # CH340 USB 시리얼 속도(Waveshare 고정값)
              bitrate=500000,          # CAN 비트레이트 500k
              frame_type="STD",
    ...
    while True:
        ...
        try:
            if c == "q":
                ...
                tx(0x010, [1])   # HMI_Emergency / SafeAbort (DBC 0x010): 1=래치 정지
            elif c == "r":
                tx(0x010, [0])   # 0=해제
            else:
                print(HELP)
