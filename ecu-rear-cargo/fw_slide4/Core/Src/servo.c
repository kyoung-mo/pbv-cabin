...
/* 서보 장착 방향 반전: 1이면 명령 X°가 물리적으로 (180-X)°로 간다.
 * 모터/기구가 반대로 장착돼 재배선 불가할 때 코드로 뒤집는다. 채널별.
 * 지령각(tgtDeg/curDeg)은 논리값 그대로 유지 → 상태보고·핀치 로직 영향 없음. */
static const uint8_t SERVO_INVERT[SERVO_COUNT] = {
  1,   /* SERVO_RL — 방향 반전(2026-07-06) */
  1,   /* SERVO_RR — 방향 반전(2026-07-06) */
};
...
/* 채널 방향 반전을 적용한 펄스폭. 논리각→물리각 변환은 여기 한 곳에서만. */
static uint32_t deg_to_us_ch(servo_ch_t ch, uint8_t deg)
{
  if (deg > SERVO_MAX_DEG) deg = SERVO_MAX_DEG;
  if (SERVO_INVERT[ch]) deg = SERVO_MAX_DEG - deg;   /* 명령 X° → 물리 (180-X)° */
  return deg_to_us(deg);
}
...
void servo_init(void)
{
  for (uint8_t ch = 0; ch < SERVO_COUNT; ch++) {
    curDeg[ch] = 0;
    tgtDeg[ch] = 0;
    __HAL_TIM_SET_COMPARE(&htim2, SERVO_TIM_CH[ch], deg_to_us_ch(ch, 0));
...
void servo_service(void)
{
  ...
  for (uint8_t ch = 0; ch < SERVO_COUNT; ch++) {
    ...
    __HAL_TIM_SET_COMPARE(&htim2, SERVO_TIM_CH[ch], deg_to_us_ch(ch, curDeg[ch]));
  }
}
