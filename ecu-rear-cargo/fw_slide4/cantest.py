...
PORT = "COM9"   # ← CANable 포트 (Nucleo VCP COM 아님!)
...
def on_rx(m):
    ...
    if aid == 0x220 and len(d) >= 3:        # Rear_Left_Seat_Status
        p = d[1] & 1
        sl = "미확정" if d[2] == 0xFF else f"{d[2]}mm"   # 0xFF=원점 미확정
        line += f"  | RL recline={d[0]} slide={sl} pinch={p}" + ("   <<<< 끼임! STOP >>>>" if p else "")
    elif aid == 0x221 and len(d) >= 3:      # Rear_Right_Seat_Status
        p = d[1] & 1
        sl = "미확정" if d[2] == 0xFF else f"{d[2]}mm"
        line += f"  | RR recline={d[0]} slide={sl} pinch={p}" + ("   <<<< 끼임! STOP >>>>" if p else "")
    print(line)
...
SLIDE_HOLD = 253 # 슬라이드 유지 센티넬(펌웨어 SLIDE_HOLD_CMD): 서보만 조정할 때 슬라이드 바이트에 사용
    ...
    while True:
        ...
        try:
            if c == "q":
                ...
                # RL 리클라인 서보만 조정: 슬라이드 바이트는 HOLD(253)로 보내 슬라이드가 안 움직이게.
                rl[0] = clamp(int(c[1:]), 0, 180); tx(0x120, [rl[0], SLIDE_HOLD])
            elif c.startswith("b"):
                # RR 리클라인 서보만 조정: 슬라이드 바이트는 HOLD(253). 카고램프([2])는 유지.
                rr[0] = clamp(int(c[1:]), 0, 180); tx(0x121, [rr[0], SLIDE_HOLD, rr[2]])
            elif c == "e":
