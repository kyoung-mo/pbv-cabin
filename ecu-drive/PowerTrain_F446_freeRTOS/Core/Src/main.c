...
/* 이전 듀티 저장용 전역 변수 */
uint32_t prev_left_duty = 0;
uint32_t prev_right_duty = 0;
/* USER CODE END PV */
...
HAL_TIM_Encoder_Start(&htim3, TIM_CHANNEL_ALL); // 오른쪽 바퀴 엔코더 감시 시작
  ...
  osThreadDef(DriveTask, StartDriveTask, osPriorityHigh, 0, 256);
...
void StartDriveTask(void const * argument)
{
  ...
  for(;;)
  {
	...
	if (is_emergency_active || cmd_watchdog_timer > 300)
	{
		...
		// ★ [Fail-Safe 가드 레일 보완] 비상 정지 복귀 시 Ramp 튐 현상을 막기 위해 백업 데이터 리셋
		prev_left_duty = 0;
		prev_right_duty = 0;

		// 위급 상황 시 즉각 응답 피드백 송신 (예외적으로 태스크 대기 없이 다이렉트 송신 처리)
		tx_drive_status.current_velocity = 0;
		tx_drive_status.current_gear_status = current_gear;
		tx_drive_status.drive_motor_current = 0;
	...
	// 3. [일반 주행 제어] 기어 상태 인터록 및 상하위 속도 연산
	...
	double K = 0.5; // 필요시 민감도를 2.5 등으로 올려서 테스트하세요!
	...
	#define MAX_DUTY_STEP 15
	// 왼쪽 바퀴 Ramp 적용
	if (left_duty > prev_left_duty) {
		if ((left_duty - prev_left_duty) > MAX_DUTY_STEP) left_duty = prev_left_duty + MAX_DUTY_STEP;
	} else if (left_duty < prev_left_duty) {
		if ((prev_left_duty - left_duty) > MAX_DUTY_STEP) left_duty = prev_left_duty - MAX_DUTY_STEP;
	}

	// 오른쪽 바퀴 Ramp 적용
	if (right_duty > prev_right_duty) {
		if ((right_duty - prev_right_duty) > MAX_DUTY_STEP) right_duty = prev_right_duty + MAX_DUTY_STEP;
	} else if (right_duty < prev_right_duty) {
		if ((prev_right_duty - right_duty) > MAX_DUTY_STEP) right_duty = prev_right_duty - MAX_DUTY_STEP;
	}
...
void StartCanTxTask(void const * argument)
{
  ...
  for(;;)
  {
    ...
    // 1. 구동 태스크와 인터럽트가 실시간 갱신하는 글로벌 전역 변수를 안전하게 로컬로 스냅샷 복사
	int16_t local_rpm = (int16_t)current_rpm;
	uint8_t local_gear = current_gear;

	// 2. 비상 정지나 타임아웃 상태가 아닐 때만 10ms 정주기 피드백 역송신 수행
	if (!is_emergency_active && cmd_watchdog_timer <= 300){
		// 피드백 데이터 빌드: 속도는 0, 현재 기어 상태 및 에러 플래그 결합 송신
		tx_drive_status.current_velocity = local_rpm;
		tx_drive_status.current_gear_status = local_gear;
		tx_drive_status.drive_motor_current = 0;

		// 0x200 Drive_Status CAN 송신 함수 호출 (10ms 주기 세이프티 송신)
		model_car_net_drive_status_pack(tx_data, &tx_drive_status, sizeof(tx_data));
